"""
Fetch squads for all iterations from Impect API and load into Snowflake
Table: CAFC_DB.IMPECT_RAW.SQUADS
"""
import pandas as pd
from impect_api import get_iterations, get_squads
...
def run():
    ...
    for iteration_id in iteration_ids:
        print(f"  Fetching squads for iteration {iteration_id}...")
        try:
            ...
            print(f"  Warning: Could not fetch squads for iteration {iteration_id}: {e}")
            continue

    if not all_frames:
        print("No squad data retrieved")
        return
