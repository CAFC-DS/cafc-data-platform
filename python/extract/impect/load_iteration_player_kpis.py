...
Output shape: one row per (iteration, squad, player, kpi).
Columns landed: ITERATION_ID, SQUAD_ID, PLAYER_ID, POSITION, PLAY_DURATION,
                MATCH_SHARE, KPI_ID, VALUE.

One API call per (iteration, squad) pair. With ~708 iterations × ~25 squads
each ≈ 17,000 calls. Parallelised with ThreadPoolExecutor.

Usage:
    python load_iteration_player_kpis.py                       # full extract
    python load_iteration_player_kpis.py --iteration-id 1410   # single iteration
    python load_iteration_player_kpis.py --limit-iterations 3  # first 3
    python load_iteration_player_kpis.py --max-workers 4
"""
import argparse
