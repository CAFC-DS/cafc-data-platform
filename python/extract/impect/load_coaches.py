"""
Fetch coaches for all iterations from Impect API and load into Snowflake
Table: CAFC_DB.IMPECT_RAW.COACHES
"""
import pandas as pd
from impect_api import get_iterations, get_coaches
...
def run():
    ...
    for iteration_id in iteration_ids:
        print(f"  Fetching coaches for iteration {iteration_id}...")
        try:
            response = get_coaches(iteration_id)
            df = json_to_dataframe(response)
            if not df.empty:
                df['ITERATION_ID'] = iteration_id
                all_frames.append(df)
        except Exception as e:
            print(f"  Warning: Could not fetch coaches for iteration {iteration_id}: {e}")
            continue

    if not all_frames:
        print("No coach data retrieved")
        return
