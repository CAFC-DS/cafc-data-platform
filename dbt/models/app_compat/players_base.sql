/*
  app_compat.players_base
  -----------------------
  Legacy-shape player snapshot — the RECRUITMENT_TEST.PUBLIC.PLAYERS contract,
  sourced from the canonical layer. One row per canonical player AS OF the
  last dbt build. The app-facing contract is the `players` VIEW, which unions
  this base with players minted in CORE since the build (Phase 5: the app
  mints manual players directly into CORE.PLAYERS and must see them
  immediately). Don't point the app here.
  ...
  that went stale (squad name doesn't move with a transfer until the player's
  next KPI-generating appearance) and would have broken outright once the
  KPI facts retire in favour of event data. NULL for players with no in-scope
  data (out-of-scope/historical/internal, or — for POSITION specifically —
  players whose matches haven't had event data backfilled yet).
...
with canonical as (
    select
        ...
        birth_date, birth_place, strong_foot
