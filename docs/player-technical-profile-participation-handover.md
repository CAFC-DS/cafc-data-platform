...
- `CAFC_DB.CORE.CORE_PLAYER_FIXTURE_POSITION_PARTICIPATION`: exact minutes at
  ...
  per-90 denominators.
- `CAFC_DB.CORE.CORE_IMPECT_PLAYER_FIXTURE_POSITION_PARTICIPATION`: source-grain
  event-derived minutes, retaining rows even when canonical identity links are
  unavailable. Use this for external Impect player endpoints.
...
`player_data[1]`. Source-grain participation and raw events use the former.
...
Bind `int(player_data[0])`, the external Impect ID.
